...
const char* DataInputStream::getPath(const cocos2d::CCString path){
    ...
    return CCFileUtils::sharedFileUtils()->fullPathFromRelativePath(path.getCString());
}
...
//     CCLOG("fullPath=%s",getPath(path));
	buffer = fUtils->getFileData(getPath(path), "rb", (unsigned long *)(&pvrlen));
//     CCLOG("open  =%s",buffer);
	if(pvrlen == 0)
	{
		return false;
